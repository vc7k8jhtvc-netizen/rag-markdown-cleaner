...
`review_required=false` 只表示程序没有触发当前启发式告警，不能替代人工抽查，也不能作为无人值守自动入库的依据。

完整文件 metadata 的 `status` 用于下游门禁：`processed` 表示宽松模式处理完成，
`review_required` 表示必须复核，只有 `approved_for_ingestion` 表示严格校验通过并可进入
自动入库流程。严格 Front Matter 校验默认启用；只有明确使用 `--no-strict` 才进入宽松模式。
...
v1.8.7 加强内容完整性校验和入库门禁，默认启用严格 Front Matter 校验，
增加锁文件 CI 与类型警告基线，并改进并发 API 客户端关闭和 Windows 重定向输出。

v1.8.5 修复块引用内带空行的围栏代码被错误拆分，以及表格后直接接 Markdown 标题时被误判为超长表格的问题。

v1.8.4 修复复核副本更新失败时误删旧复核文件的问题；如果新报告写入失败，会恢复更新前的有效复核材料。
