...
完整结果的 `status` 是下游消费门禁：

- `processed`：宽松模式已完成处理，但尚未获准自动入库；
- `review_required`：存在校验或质量警告，必须人工复核；
- `approved_for_ingestion`：严格 Front Matter 校验及当前质量门禁均已通过。

旧版 `completed` 状态会触发重新组装，以便按当前门禁重新分类；已完成且身份匹配的
分片不会因此重复调用 API。
