...
各版本条目记录对应版本发布时的历史事实，不代表当前分支、当前测试数量或尚未发布功能；
当前版本以 `pyproject.toml` 为准，未发布变更只记录在 `[Unreleased]`。

## [1.8.7] - 2026-07-29

### Fixed

- 严格 Front Matter 校验改为默认启用；校验警告现在必然触发人工复核，最终产物使用 `processed`、`review_required` 和 `approved_for_ingestion` 区分处理完成与可入库状态。
- 质量检查现在按规范化后的数字、百分比、工程单位、标准编号和 URL 出现顺序保护正文，并以稳定文本锚点阻止长度和结构近似的大范围内容替换；广告行仍可按既有规则删除。
- API 客户端关闭前会等待活动流式请求结束，避免并发停止或退出期间底层 HTTP 客户端被提前关闭。
- 质量失败候选和运行日志增加失败阶段、响应完整性、输入输出字符数及保留率诊断，可区分接口截断、token 上限与完整响应的质量拦截。
- Windows 非交互输出现在与宿主控制台输出代码页保持一致，修复 PowerShell 5.1 将 `--help` 重定向到文件时的中文乱码。

### Documentation

- 明确运行数据采用生命周期保留且不按固定时间自动删除，并将旧开发日志标记为历史归档快照。

### Validation

- CI 增加基于 `uv.lock` 的冻结环境，并以 491 条历史警告为 basedpyright 基线，阻止新增类型警告；发布校验工具使用精确版本。
- PowerShell 5.1 和 7.6.4 均通过三个脚本的语法解析与中文菜单启动/退出冒烟；未启动处理任务，未调用 API。
...
- 修复块引用内带空行的围栏代码被错误拆分、导致 Markdown 围栏结构损坏的问题。
- 修复表格后直接接 Markdown 标题时被误判为一个超长表格、导致切片规划失败的问题。

## [1.8.4] - 2026-07-26

### Fixed

- 更新复核副本时，如果 JSON 报告写入失败，会恢复更新前仍然有效的 Markdown 和 JSON 文件；首次创建失败时只清理本次新建的文件。
